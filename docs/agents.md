...
- **Risk Agent** supports exception handling by identifying blocked work, escalations, high-impact risks, and at-risk milestones.
- **Dependency Agent** supports operations coordination and blocker identification across Jira and RAID artefacts.
- **Governance Agent** supports policy and control checks for ownership, mitigations, acceptance criteria, and release readiness.
- **Recommendation Agent** supports human-in-loop task routing and leadership actions by prioritising evidence-backed recommendations.
- **Executive Summary Agent** supports reporting and evidence by converting scored findings into an executive summary and decision prompt.

## Guardrails and Human-in-the-loop Controls

Delivery Chief of Staff is designed as an advisory governance workflow for banking delivery and shared services reviews.

- The deterministic delivery governance engine is the source of truth.
- Delivery Health Score, status, findings, evidence keys and score deductions are generated by deterministic agents.
- Optional OpenAI enhancement, if enabled, is limited to executive narrative generation.
- OpenAI must not alter the score, status, findings, evidence keys, score deductions or recommended actions unless supported by existing deterministic findings.
- The system does not autonomously approve delivery decisions, release decisions, governance exceptions or escalations.
...
- The MVP processes uploaded CSVs locally and does not require direct system execution across Jira, release tools or banking systems.
- If OpenAI is unavailable or disabled, the deterministic executive summary and report remain available.
- Secrets such as `OPENAI_API_KEY` must be provided through environment variables or a local `.env` file that is not committed.
