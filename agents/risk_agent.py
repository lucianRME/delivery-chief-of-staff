...
def analyze_risks(jira: pd.DataFrame, raid: pd.DataFrame, release: pd.DataFrame) -> list[dict]:
    ...
    for _, row in jira.iterrows():
        ...
        if _is(status, "Blocked"):
            ...
            findings.append(_finding(
                severity, "Blocked Issue", "Delivery Risk",
                f"{priority or 'Unprioritised'} issue {issue} is blocked.",
                f"issue_key={issue}; status={status}; blocker={_text(row.get('blocker')) or 'not specified'}",
                evidence_key, "Jira",
                f"Assign an owner to remove the blocker for {issue} and confirm a recovery date.",
            ))
        if _is(priority, "Critical", "Highest", "High") and not _is(status, "Done", "Closed", "Resolved"):
            severity = "Critical" if _is(priority, "Critical", "Highest") else "High"
            findings.append(_finding(
                severity, "Priority Exposure", "Delivery Risk",
                f"{priority} priority issue {issue} is not complete.",
                f"issue_key={issue}; priority={priority}; status={status or 'missing'}",
                evidence_key, "Jira",
    ...
    for _, row in raid.iterrows():
        ...
        if _is(item_type, "Risk", "Issue") and _is(impact, "High", "Critical") and not _is(status, "Closed", "Resolved"):
            findings.append(_finding(
                "Critical" if _is(impact, "Critical") else "High", "High-impact RAID Item", "Delivery Risk",
                f"High-impact {item_type.lower()} {raid_id} remains open.",
                f"raid_id={raid_id}; impact={impact}; status={status or 'missing'}",
                evidence_key, "RAID",
                f"Confirm mitigation, accountable owner, and target resolution for {raid_id}.",
            ))
        if _is(row.get("escalation_required"), "Yes", "Y", "True"):
            findings.append(_finding(
                "High", "Escalation Required", "Governance",
                f"RAID item {raid_id} requires escalation.",
                f"raid_id={raid_id}; escalation_required={_text(row.get('escalation_required'))}",
                evidence_key, "RAID",
                f"Escalate {raid_id} to the appropriate governance forum with a clear decision request.",
            ))
    ...
    for _, row in release.iterrows():
        ...
        if _is(row.get("milestone_status"), "At Risk"):
            findings.append(_finding(
                "High", "Milestone At Risk", "Release Readiness",
                f"Release milestone '{milestone}' is at risk.",
                f"release_id={release_id}; milestone={milestone}; milestone_status={_text(row.get('milestone_status'))}",
                evidence_key, "Release",
                f"Create a recovery plan for '{milestone}' and assess impact on the release date.",
            ))
